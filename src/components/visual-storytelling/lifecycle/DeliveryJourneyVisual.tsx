...
import {
  DELIVERY_JOURNEY_LAYOUT,
  homeDeliveryJourneyCopy,
} from "@/config/home-delivery-journey";
import { useReducedMotion } from "@/motion/useReducedMotion";
import { cn } from "@/lib/cn";

type StageId = (typeof homeDeliveryJourneyCopy.stages)[number]["id"];

const STAGE_IDS = homeDeliveryJourneyCopy.stages.map((stage) => stage.id);

const iconPaths: Record<
  (typeof homeDeliveryJourneyCopy.stages)[number]["icon"],
  ReactNode
> = {
  discover: (
    <path
...
function StageIcon({
  ...
  icon: (typeof homeDeliveryJourneyCopy.stages)[number]["icon"];
  className?: string;
}) {
  return (
    <svg
...
export function DeliveryJourneyVisual({ className }: { className?: string }) {
  const { hovered, setHovered, bindNode, clearHover } = useStoryVisualHover<StageId>();
  const reducedMotion = useReducedMotion();
  const { viewBox, trackY, nodeY, nodeRadius, stages } = DELIVERY_JOURNEY_LAYOUT;
  const activeStage =
    homeDeliveryJourneyCopy.stages.find((stage) => stage.id === hovered) ?? null;
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        <StoryVisualDiagram
          viewBox={viewBox}
          ariaLabel="Delivery journey from discover through design, build, AI integration, launch, to scale"
        >
          <defs>
            <linearGradient id="delivery-track-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="#06b6d4" stopOpacity="0.9" />
              <stop offset="25%" stopColor="#3b82f6" stopOpacity="0.85" />
              <stop offset="50%" stopColor="#8b5cf6" stopOpacity="0.85" />
              <stop offset="75%" stopColor="#d4a017" stopOpacity="0.9" />
              <stop offset="100%" stopColor="#06b6d4" stopOpacity="0.85" />
            </linearGradient>
            <StoryVisualGlowFilter id="delivery-node-glow" stdDeviation={3} />
          </defs>
          ...
          <line
            ...
            x2={lastX}
            y2={trackY}
            stroke="url(#delivery-track-gradient)"
            strokeWidth={2}
            strokeLinecap="round"
          ...
          {!reducedMotion && !hovered ? (
            <StoryVisualTravelDot
              ...
              duration={6}
              r={4}
              filter="url(#delivery-node-glow)"
            />
          ) : null}

          {homeDeliveryJourneyCopy.stages.map((stage) => {
            ...
            return (
              <StoryVisualInteractiveGroup
                key={stage.id}
                ariaLabel={`${stage.label}: ${stage.outcome}`}
                {...bindNode(stage.id)}
              >
                <StoryVisualNodeCircle
                  cx={layout.x}
        ...
        <StoryVisualDetailPanel placeholder="Hover a stage to see how Nexynth turns ideas into production systems.">
          {activeStage ? (
        ...
        <StoryVisualMobileJourney
          steps={homeDeliveryJourneyCopy.stages.map((stage, index) => ({
            id: stage.id,
            label: stage.label,
