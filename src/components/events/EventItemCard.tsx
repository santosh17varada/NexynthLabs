...
export function EventItemCard({ item }: { item: EventItem }) {
  return (
    <Card as="article" className="flex h-full flex-col p-5 sm:p-6">
      ...
      <p className="mt-2 text-sm font-medium text-foreground">{item.schedule}</p>
      <p className="text-xs text-muted">{item.format}</p>
